...
export const useDrawerConversationStore = defineStore(
  'drawer-conversation-store',
  () => {
    let conversationList = reactive([])
    let curConversationId = ref('')
    ...
    function createConversation(title) {
      const conversation = {
        ...getDefaultConversation(title, DrawerChatBotId)
      }
      conversationList.push(conversation)
      window.localStorage.setItem('drawer-conversation-list', JSON.stringify(conversationList))
      curConversationId.value = conversation.id
      return conversation.id
    }
  ...
  },
  {
    persist: {
      paths: ['curConversationId'],
      afterRestore: (ctx) => {
        const storageCache = window.localStorage.getItem('drawer-conversation-list')
        if (storageCache) {
          ctx.store.conversationList.push(...JSON.parse(storageCache))
        }
      }
